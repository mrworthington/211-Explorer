...
body <- dashboardBody(
  tags$head(
    tags$link(rel = "stylesheet", type = "text/css", href = "whitney_font.css"),
    tags$link(rel = "stylesheet", type = "text/css", href = "selectizebootstrap.css"),
    tags$link(rel="stylesheet", href="https://cloud.typography.com/6771036/7556012/css/fonts.css"),
...
server <- function (input, output, session) {
  ...
  output$treemapcallexplore <- renderHighchart({
    ...
    #   select(Year, zip_code, gender, need_name, count) %>%
    #   group_by_at(vars(1,2,3,4)) %>% 
    #   summarize(value = sum(count)) %>%
    #   rename(col_2=2,col_3=3,col_4=4) %>% 
    #   filter(col_2!="Undetermined" & col_2!="Undetermined") %>%
    #   top_n(20, value) %>%
    #   ungroup() %>%
    #   as_tibble()
    ...
    hctreemap2(treemap_explore1,
               group_vars = c("col_2","col_3","col_4"),
               size_var = "value",
               color_var = "value",
               animation = FALSE,
               opacity = .4,
      ...
      hc_add_theme(hc_theme(
        chart = list(style = list(fontFamily = "Whitney SSm A, Whitney SSm B",
                                  fontSize = "16px")),
        plotOptions = list(
          series = list(
            dataLabels = list(style = list(fontFamily = "Whitney SSm A, Whitney SSm B",
                                           fontSize = "1.3em",
                                           textOutline = FALSE)))
          ))) %>%
  ...
  output$map <- renderLeaflet({
    ...
    output$zip_profile <- renderHighchart({
      ...
      hchart(travis_profile, "column", hcaes(x = measures, y = value), animation = FALSE) %>% 
        ...
        hc_plotOptions(
          series = list(
            dataLabels = list(fontFamily = "Whitney SSm A, Whitney SSm B",
                              format = "{y}%",
                              enabled = TRUE,
                              borderRadius = 5,
        ...
        hc_add_theme(
          hc_theme_merge(
            ...
            hc_theme(chart = list(
              ...
              style = list(fontFamily = "Whitney SSm A, Whitney SSm B")))))
